...
def dump_data(**kwargs):
    for table in ['orderdetails', 'orders', 'products', 'productsuppl', 'suppliers']:
        phook = PostgresHook(postgres_conn_id="source_db")
...
def input_data(**kwargs):
    for table in ['orderdetails', 'orders', 'products', 'productsuppl', 'suppliers']:
        phook = PostgresHook(postgres_conn_id="target_db")
